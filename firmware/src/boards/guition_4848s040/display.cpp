...
// Do not call rgbpanel->getFrameBuffer() after gfx->begin() — it constructs
// a second RGB panel and crashes.
...
// No button to cycle brightness here, so the shared default level (200) is
// stretched to full backlight; lower levels during idle fades scale with it.
void display_hal_set_brightness(uint8_t level) {
    uint16_t duty = (uint16_t)level * 255 / 200;
    ledcWrite(LCD_BL, duty > 255 ? 255 : duty);
}
