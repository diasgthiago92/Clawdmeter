...
def test_instagram_states_and_stories_left_out(tmp_path):
    rows = build_rows([
        item("2026-09-19", 19, "noite", "tweet-problema-repo", "publicado"),   # ontem: fora da janela
        item("2026-09-20", 8, "manha", "tweet-problema-repo", "publicado"),
        item("2026-09-20", 7, "cedo", "tweet-problema-repo", "pendente"),
        item("2026-09-21", 12, "almoco", "imagem-problema-repo", "agendado"),
        item("2026-09-21", 13, "story-solucao", "story", "agendado"),
    ], [], tmp_path, TODAY)
    assert [(r[0], r[1], r[3]) for r in rows] == [
        (INSTAGRAM, "20/09 07h", NAO_ENVIADO),      # ordenado por data
        (INSTAGRAM, "20/09 08h", ENVIADO),
        (INSTAGRAM, "21/09 12h", AGENDADO),
    ]


def test_reel_goes_to_tiktok_only_when_logged(tmp_path):
    (tmp_path / "logs").mkdir()
    (tmp_path / "logs" / "2026-09-20-extra.log").write_text("TikTok: rascunho enviado (videos/x)\n")
    rows = build_rows([
        item("2026-09-20", 10, "extra", "reel-pronto", "publicado", "Com log"),
        item("2026-09-21", 10, "extra", "reel-pronto", "publicado", "Sem log"),
    ], [], tmp_path, TODAY)
...
def test_queued_resend_replaces_the_derived_tiktok_row(tmp_path):
    fila = [{"quando": "2026-09-21 10:00", "video": "v.mp4", "titulo": "Reenvio",
             "post": "content/published/2026-09-20-extra/post.json"}]
    rows = build_rows([item("2026-09-20", 10, "extra", "reel-pronto", "publicado", "Reel")], fila, tmp_path, TODAY)
...
def test_window_and_title_folding(tmp_path):
    rows = build_rows([
        item("2026-09-19", 8, "manha", "curiosidade-ia", "publicado"),
        item("2026-09-22", 8, "manha", "curiosidade-ia", "agendado"),
